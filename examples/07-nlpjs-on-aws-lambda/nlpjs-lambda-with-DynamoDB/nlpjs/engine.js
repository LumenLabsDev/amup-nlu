...
function checkModel() {
  ...
  try {
    const description = dynamoDB.describeTable(
      params,
      function (errDesc, dataDesc) {
        if (errDesc) {
          console.error(
            "THERE IS A BIG PROBLEM! SEEMS THAT THE TABLE WHICH MUST CONTAIN THE MODEL DOESN'T EXIST: ",
            errDesc
          );
        ...
        if (dataDesc.Table.ItemCount === 0) {
          ...
          // dataDesc.Table.ItemCount > 0
          console.info(
            `THE TABLE WITH A MODEL ALREADY EXISTS. THERE IS ${dataDesc.Table.ItemCount} ITEMS`
          );

...
```sh
npm i botbuilder express @lumen-labs-dev/node-nlp
```
...
const fs = require('fs');
const { Recognizer } = require('@lumen-labs-dev/node-nlp');
...
// Creates a @lumen-labs-dev/node-nlp recognizer for the bot
...
// use the @lumen-labs-dev/node-nlp recognizer to calculate the answer.

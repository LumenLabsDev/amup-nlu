...
**- But this example seems to be of the v3...** Version 4 is split into different smaller packages, but the https://www.npmjs.com/package/@lumen-labs-dev/node-nlp package uses those smaller packages to build a version as retrocompatible with v3 as we can
...
**- But this is for backend... I want this bundle to work in my browser** You have the quickstart for browser bundles here: https://github.com/axa-group/nlp.js/blob/master/docs/v4/webandreact.md
...
**- And how I guess a language from a sentence, not integrated with the NLP?** You have the example here: https://github.com/axa-group/nlp.js/blob/master/docs/v3/language-guesser.md But if you want to get an smaller impact on your node_moules use the library _@lumen-labs-dev/language_ instead of _@lumen-labs-dev/node-nlp_ one.
...
```js
const { NGrams } = require('@lumen-labs-dev/utils');
const fs = require('fs');
...
```js
const { composeFromPattern, composeCorpus } = require('@lumen-labs-dev/utils');
...
```js
const { similarity } = require('@lumen-labs-dev/similarity');
...
```js
const { ExtractorEnum } = require('@lumen-labs-dev/ner');

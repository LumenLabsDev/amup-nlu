...
This is a JavaScript monorepo for NLP.js-derived packages published under the **`@lumen-labs-dev/*`** npm scope. All installable packages use this prefix (for example `@lumen-labs-dev/basic`, `@lumen-labs-dev/lang-en-us`). Packages live under `packages/*` and are managed with npm workspaces plus Lerna metadata.
...
- `@lumen-labs-dev/basic` is the recommended entry point for new work (core + common plugins).
- `@lumen-labs-dev/lang-en-us` is included by `@lumen-labs-dev/basic`; other language packages load on demand, and `@lumen-labs-dev/lang-all` is optional for all-language compatibility.
...
```bash
npm run test:jest -- packages/nlp/test packages/lang-all/test
```
